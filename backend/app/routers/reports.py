...
async def pnl(
    ...
    # 260 «Прочие налоги и сборы от прибыли»: реально ОПЛАЧЕННЫЕ (не
    # начисленные) суммы НДФЛ/ЕСП/ИНПС/земельного/прочих налогов — как в
    # книге, лист «Налоги», D53 = SUM(F8:F13) (F — «оплачено», формулы
    # тянутся из листа CASH FLOW). НДС сюда не входит (у него своя строка
    # через 010/020), налог на прибыль — тоже (он уже в строке 250 выше,
    # начисленный, а не оплаченный — не дублируем). Общефирменная строка,
    # как и 250 (в книге у неё тоже нет разбивки по подразделениям).
    other_taxes = 0.0
    if not division:
        tdata = await _taxes_core(db, year, month)
        for r in tdata["rows"]:
            n = r["name"].lower()
            if "ндс" in n or "прибыль" in n:
                continue
            other_taxes += r["paid"]
