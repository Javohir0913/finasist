...
#
# Полученные УСЛУГИ стр.13-15 — «UCON IMPERIUM», ведение бухучёта за июль
# (8+8+11.5 млн), книга проставила «Объект»=Махстон всем трём (включая
# ИНН «...(ЖБИ)» и «...(АЗМУР)» — это разные договоры/юрлица, не объекты
# производства). По прямому указанию пользователя — это не расход конкретного
# подразделения, а общий (административный) расход: убираем «Объект» (пустая
# строка — не попадает ни в один из трёх «ОФР <объект>», только в общий отчёт).
ROW_DIVISION_OVERRIDE = {
    ("КАССА", 800): "Махстон",
    ("Полученные УСЛУГИ", 61): "Махстон",
    ("Полученные УСЛУГИ", 13): "",
    ("Полученные УСЛУГИ", 14): "",
    ("Полученные УСЛУГИ", 15): "",
}
_NO_OVERRIDE = object()
    ...
    def services_received(self) -> list[dict]:
        ...
        for i, r in self.rows("Полученные УСЛУГИ", 6, None, 14):
            ...
            if not d or (not net and not vat_amt):
                continue
            override = ROW_DIVISION_OVERRIDE.get(("Полученные УСЛУГИ", i), _NO_OVERRIDE)
            division = txt(r[9]) if override is _NO_OVERRIDE else override
            out.append({
