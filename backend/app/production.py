...
# 2010 — итоговая строка книги, 2011 — «Стоимость приобретённых ТМЗ».
# Списание с этими кодами (и без кода) — прямой расход сырья, а не «общие
# расходы»: иначе одна и та же сумма попала бы в себестоимость дважды.
DIRECT_GROUPS = ("subtotal", "asset")
...
async def cost_parts(
    ...
    # 1) списание со склада: без кода и 2010/2011 — прямой расход сырья,
    #    производственные коды — «общие расходы» (колонка H листа)
    istmt = by_div(
    ...
    for code, uzs in (await db.execute(period(istmt, MaterialIssue.doc_date))).all():
        ...
        if code and code in prod_codes:
            stock_prod += v
        elif not code or mapping.get(code, "admin") in DIRECT_GROUPS:
            materials += v
