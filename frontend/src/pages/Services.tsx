...
export default function Services() {
  ...
  const openNew = () => { setEditing(null); setForm({ ...empty, direction: dir }); setErr(""); setOpen(true); };
  const openEdit = (s: Svc) => {
    setEditing(s);
    setForm({ ...empty, ...s, organization_id: s.organization ? String(s.organization.id) : "" });
    setErr(""); setOpen(true);
  };
  const save = async () => {
    setErr(""); setSaving(true);
    const body = { ...form, direction: editing ? form.direction : dir, organization_id: form.organization_id ? Number(form.organization_id) : null, amount: Number(form.amount) };
    try {
    ...
    <div>
      <SectionTitle title="Услуги" sub="Полученные и оказанные услуги (с расчётом НДС)"
        right={<div className="flex flex-wrap items-center gap-2">
          <PeriodPicker />
          {can("services:create") && <button className="btn-primary" onClick={openNew}>+ Услуга</button>}
        </div>} />
              ...
              {data.map((s) => (
                <tr key={s.id} className="hover:bg-veil/[0.02]">
                  ...
                  <td className="td text-right whitespace-nowrap">
                    {isLocked(s.doc_date) ? <LockedMark title={hint} /> : <>
                      {can("services:edit") && <button onClick={() => openEdit(s)} className="text-slate-500 hover:text-accent-soft mr-3">✎</button>}
                      {can("services:delete") && <button onClick={() => remove(s.id)} className="text-slate-500 hover:text-rose-300">✕</button>}
                    </>}
                  </td>
                </tr>
              ))}
      ...
      <Modal open={open} onClose={() => { setOpen(false); setEditing(null); }}
        title={editing ? `Редактировать №${editing.id}` : form.direction === "received" ? "Полученная услуга" : "Оказанная услуга"} width="max-w-xl">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          {form.direction === "received" && <Field label="Код расхода"><SearchSelect value={form.expense_code} onChange={(v) => setForm({ ...form, expense_code: v })} placeholder="—" emptyLabel="—" options={(expCodes || []).map((c) => ({ value: c.code, label: `${c.code} · ${c.name}` }))} /></Field>}
        ...
        <div className="flex justify-end gap-2 mt-6"><button className="btn-ghost" onClick={() => { setOpen(false); setEditing(null); }}>Отмена</button><button className="btn-primary" onClick={save} disabled={saving}>Сохранить</button></div>
      </Modal>
    </div>
  );
}
